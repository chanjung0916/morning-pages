...
function buildPrompt(keyword: string): string {
  return (
    `아침 일기 앱에서 사용할 '${keyword}' 주제의 글쓰기 프롬프트 5가지를 만들어줘.\n\n` +
    `아침 일기의 핵심 개념:\n` +
    `• 하루가 시작되기 전, 오늘 하루를 내가 직접 설계하는 일기\n` +
    `• 어제를 돌아보는 게 아니라 오늘을 어떻게 살지 결정하는 것\n` +
    `• 읽고 나면 "오늘을 어떻게 보낼지 알겠다"는 느낌이 들어야 함\n\n` +
    `조건:\n` +
    `• 시제는 오늘/지금/앞으로에 집중 (과거형 금지)\n` +
    `• 읽는 사람이 스스로 선택하고 결정하게 유도하는 질문\n` +
    `• 1~2문장으로 짧고 명확하게\n` +
    `• 가볍고 부드러운 톤 (부담 없이 시작할 수 있게)\n` +
    `• 한국어로 작성\n\n` +
    `형식 (콜론으로 구분):\n` +
    `주제 제목: 안내 문장\n` +
    `주제 제목: 안내 문장\n` +
